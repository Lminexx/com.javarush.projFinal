...
CREATE TABLE PROJECT (ID BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT ,CODE VARCHAR(32) NOT NULL UNIQUE,TITLE VARCHAR(1024) NOT NULL,DESCRIPTION VARCHAR(4096) NOT NULL,TYPE_CODE VARCHAR(32) NOT NULL,STARTPOINT TIMESTAMP,ENDPOINT TIMESTAMP,PARENT_ID BIGINT,CONSTRAINT FK_PROJECT_PARENT FOREIGN KEY (PARENT_ID) REFERENCES PROJECT (ID) ON DELETE CASCADE
);

CREATE TABLE MAIL_CASE (ID BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT,EMAIL VARCHAR(255) NOT NULL,NAME VARCHAR(255) NOT NULL,DATE_TIME TIMESTAMP NOT NULL,RESULT VARCHAR(255) NOT NULL,TEMPLATE VARCHAR(255) NOT NULL
);

CREATE TABLE SPRINT (ID BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT,STATUS_CODE VARCHAR(32) NOT NULL,STARTPOINT TIMESTAMP,ENDPOINT TIMESTAMP,TITLE VARCHAR(1024) NOT NULL,PROJECT_ID BIGINT NOT NULL,CONSTRAINT FK_SPRINT_PROJECT FOREIGN KEY (PROJECT_ID) REFERENCES PROJECT (ID) ON DELETE CASCADE
);

CREATE TABLE REFERENCE (ID BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT,CODE VARCHAR(32) NOT NULL,REF_TYPE SMALLINT NOT NULL,ENDPOINT TIMESTAMP,STARTPOINT TIMESTAMP,TITLE VARCHAR(1024) NOT NULL,AUX VARCHAR(255),CONSTRAINT UK_REFERENCE_REF_TYPE_CODE UNIQUE (REF_TYPE, CODE)
);

CREATE TABLE USERS (ID BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT,DISPLAY_NAME VARCHAR(32) NOT NULL UNIQUE,EMAIL VARCHAR(128) NOT NULL UNIQUE,FIRST_NAME VARCHAR(32) NOT NULL,LAST_NAME VARCHAR(32),PASSWORD VARCHAR(128) NOT NULL,ENDPOINT TIMESTAMP,STARTPOINT TIMESTAMP
);
...
CREATE TABLE CONTACT (ID BIGINT NOT NULL,CODE VARCHAR(32) NOT NULL,VALUE_Contact VARCHAR(256) NOT NULL,PRIMARY KEY (ID, CODE),CONSTRAINT FK_CONTACT_PROFILE FOREIGN KEY (ID) REFERENCES PROFILE (ID) ON DELETE CASCADE
);

CREATE TABLE TASK (ID BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE USER_BELONG (ID BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE ATTACHMENT (ID BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT,
...
-- Inserting data with AUX column
...
CREATE UNIQUE INDEX UK_SPRINT_PROJECT_CODE ON SPRINT(CODE);
